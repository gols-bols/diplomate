...
outfile = Path(sys.argv[2]) if len(sys.argv) > 2 else Path('docs/Диплом - правки_исправлено_final6.docx')
...
for p in doc.paragraphs:
    ...
    if any(x in style.lower() for x in ('list', 'марки', 'нумер')):
        ...
        if text and not text.startswith('—'):
            # prepend em-dash
            p.text = '— ' + text
            # remove list style — set to Normal
            p.style = 'Normal'
            changed += 1

print('Converted', changed, 'list paragraphs to em-dashes and removed styles')
